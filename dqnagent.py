...
from model import DQNetworkConv, DQNetwork
...
class Agent():
    ...
    def __init__(self, state_size, action_size, train, seed, modeltype, dqntype = "DQN"):
        ...
        if self.train:
            ...
            if self.t_step == 0:
                ...
                if len(self.memory) > BATCH_SIZE:
                    ...
                    #print([i.shape for i in experiences])
    ...
    def learn(self, experiences, gamma):
        ...
        if self.dqntype == "DDQN":
            Q_expected = self.qnetwork_local(states).gather(-1, actions)
            with torch.no_grad():
                Q_actions = self.qnetwork_local(next_states).detach().argmax(-1, keepdim=True)
